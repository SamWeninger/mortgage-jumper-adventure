...
const GameEngine: React.FC<GameEngineProps> = ({ onExit }) => {
      ...
      if (!enemy.hit) {
        if (enemy.type === 'luxury') {
          ...
          ctx.fillText(
            "$",
            Math.floor(enemy.x + enemy.width / 2),
            Math.floor(enemy.y + enemy.height / 1.8 + 4)
          );
          ...
          ctx.beginPath();
          ctx.moveTo(Math.floor(enemy.x), Math.floor(enemy.y + enemy.height));
          ctx.lineTo(Math.floor(enemy.x + enemy.width / 2), Math.floor(enemy.y));
          ...
          ctx.fillText(
            "!",
            Math.floor(enemy.x + enemy.width / 2),
            Math.floor(enemy.y + enemy.height - 10)
          );
          ...
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.moveTo(Math.floor(enemy.x + enemy.width / 2), Math.floor(enemy.y + 10));
          ctx.lineTo(Math.floor(enemy.x + enemy.width / 2), Math.floor(enemy.y + enemy.height - 20));
          ctx.moveTo(Math.floor(enemy.x + enemy.width / 2 - 8), Math.floor(enemy.y + enemy.height - 30));
          ctx.lineTo(Math.floor(enemy.x + enemy.width / 2), Math.floor(enemy.y + enemy.height - 20));
          ctx.lineTo(Math.floor(enemy.x + enemy.width / 2 + 8), Math.floor(enemy.y + enemy.height - 30));
          ctx.stroke();
        }
      }
    });
  ...
  const drawFinishLine = (ctx: CanvasRenderingContext2D) => {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        ctx.fillRect(
          Math.floor(x + col * squareSize),
          Math.floor(y + row * squareSize),
          squareSize,
          squareSize
        );
      }
    ...
    ctx.beginPath();
    ctx.moveTo(Math.floor(x + width / 2 - 5), Math.floor(y - 30));
    ctx.lineTo(Math.floor(x + width / 2 - 5), Math.floor(y - 10));
    ctx.lineTo(Math.floor(x + width / 2 - 25), Math.floor(y - 20));
    ctx.closePath();
    ctx.fill();
  };
  ...
  const drawPlayer = (ctx: CanvasRenderingContext2D) => {
    const { x, y, width, height, isDucking } = gameStateRef.current.player;
    ...
    ctx.fillRect(
      Math.floor(x + width * 0.65),
      Math.floor(y + height * 0.2),
      Math.floor(width * 0.1),
      Math.floor(height * 0.1)
    );
    
    // Mouth (smile or worried depending on ducking)
    ...
    // Tie
    ctx.beginPath();
    ctx.moveTo(Math.floor(x + width * 0.5), Math.floor(y + height * 0.5));
    ctx.lineTo(Math.floor(x + width * 0.6), Math.floor(y + height * 0.7));
    ctx.lineTo(Math.floor(x + width * 0.5), Math.floor(y + height * 0.9));
    ctx.lineTo(Math.floor(x + width * 0.4), Math.floor(y + height * 0.7));
    ctx.closePath();
    ctx.fillStyle = "#F44336"; // Red
    ctx.fill();
  };
  ...
  const drawHUD = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.font = "bold 16px Arial";
    ctx.textAlign = "left";
    ctx.fillText("Money: " + new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
    ...
    const progress = Math.min(
      gameStateRef.current.player.x / (gameStateRef.current.levelLength - 100),
      1
    );
    
    // Progress bar background
    ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
    ctx.fillRect(GAME_WIDTH - 210, 10, 200, 20);
    
    // Progress bar fill
    ctx.fillStyle = "#4CAF50";
    ctx.fillRect(GAME_WIDTH - 210, 10, 200 * progress, 20);
    ...
    ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
    ctx.fillRect(10, 60, 150, 60);
    
    ctx.fillStyle = "#FFFFFF";
    ctx.font = "14px Arial";
    ctx.textAlign = "left";
    ctx.fillText("Coins: " + gameStateRef.current.collectedCoins, 20, 80);
    ctx.fillText("Powerups: " + gameStateRef.current.collectedPowerups, 20, 105);
  };
